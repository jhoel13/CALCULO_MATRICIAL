...
export default function LabApp({ user, signInPath, signOutPath }: { user: User; signInPath: string; signOutPath: string }) {
  ...
  const applyConcreteMaterial = (ePa: number) => {
    setMaterials((current) => current.map((material) => material.id === "concrete" ? { ...material, ePa } : material));
